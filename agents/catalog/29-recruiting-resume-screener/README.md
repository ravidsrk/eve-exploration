...
Recruiting resume screener is a real-world eve archetype for Recruiting.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/29-recruiting-resume-screener 3229 "Review the current recruiting resume screener queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 29-recruiting-resume-screener
```
...
> Review the current recruiting resume screener queue and write a prioritized action report.
...
cd agents/catalog/29-recruiting-resume-screener && npx eve eval --strict   # needs keys in .secrets/eve.env
```
