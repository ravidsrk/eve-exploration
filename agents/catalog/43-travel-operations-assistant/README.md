...
Travel operations assistant is a real-world eve archetype for Travel Ops.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/43-travel-operations-assistant 3243 "Review the current travel operations assistant queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 43-travel-operations-assistant
```
...
> Review the current travel operations assistant queue and write a prioritized action report.
...
cd agents/catalog/43-travel-operations-assistant && npx eve eval --strict   # needs keys in .secrets/eve.env
```
