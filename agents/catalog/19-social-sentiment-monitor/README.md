...
Social sentiment monitor is a real-world eve archetype for Growth.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/19-social-sentiment-monitor 3219 "Review the current social sentiment monitor queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 19-social-sentiment-monitor
```
...
> Review the current social sentiment monitor queue and write a prioritized action report.
...
cd agents/catalog/19-social-sentiment-monitor && npx eve eval --strict   # needs keys in .secrets/eve.env
```
