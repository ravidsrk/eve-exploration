...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/25-contract-clause-reviewer 3225 "Review the current contract clause reviewer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 25-contract-clause-reviewer
```
...
cd agents/catalog/25-contract-clause-reviewer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
