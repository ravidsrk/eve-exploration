...
Product feedback synthesizer is a real-world eve archetype for Product.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/34-product-feedback-synthesizer 3234 "Review the current product feedback synthesizer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 34-product-feedback-synthesizer
```
...
> Review the current product feedback synthesizer queue and write a prioritized action report.
...
cd agents/catalog/34-product-feedback-synthesizer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
