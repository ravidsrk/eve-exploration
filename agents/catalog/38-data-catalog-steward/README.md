# Data catalog steward

## Rationale

Data catalog steward is a real-world eve archetype for Data Governance.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/38-data-catalog-steward 3238 "Review the current data catalog steward queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 38-data-catalog-steward
```
...
> Review the current data catalog steward queue and write a prioritized action report.
...
cd agents/catalog/38-data-catalog-steward && npx eve eval --strict   # needs keys in .secrets/eve.env
```
