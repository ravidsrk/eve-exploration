...
Ecommerce merchandising analyst is a real-world eve archetype for Commerce.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/45-ecommerce-merchandising-analyst 3245 "Review the current ecommerce merchandising analyst queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 45-ecommerce-merchandising-analyst
```
...
> Review the current ecommerce merchandising analyst queue and write a prioritized action report.
...
cd agents/catalog/45-ecommerce-merchandising-analyst && npx eve eval --strict   # needs keys in .secrets/eve.env
```
