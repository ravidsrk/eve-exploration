...
Brand voice reviewer is a real-world eve archetype for Marketing.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/18-brand-voice-reviewer 3218 "Review the current brand voice reviewer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 18-brand-voice-reviewer
```
...
> Review the current brand voice reviewer queue and write a prioritized action report.
...
cd agents/catalog/18-brand-voice-reviewer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
