...
Release notes drafter is a real-world eve archetype for Product Marketing.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/13-release-notes-drafter 3213 "Review the current release notes drafter queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 13-release-notes-drafter
```
...
> Review the current release notes drafter queue and write a prioritized action report.
...
cd agents/catalog/13-release-notes-drafter && npx eve eval --strict   # needs keys in .secrets/eve.env
```
