...
Employee IT helpdesk is a real-world eve archetype for IT.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/30-employee-it-helpdesk 3230 "Review the current employee it helpdesk queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 30-employee-it-helpdesk
```
...
> Review the current employee it helpdesk queue and write a prioritized action report.
...
cd agents/catalog/30-employee-it-helpdesk && npx eve eval --strict   # needs keys in .secrets/eve.env
```
