...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/15-test-failure-debugger 3215 "Review the current test failure debugger queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 15-test-failure-debugger
```
...
cd agents/catalog/15-test-failure-debugger && npx eve eval --strict   # needs keys in .secrets/eve.env
```
