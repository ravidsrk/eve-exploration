# Healthcare intake summarizer

## Rationale

Healthcare intake summarizer is a real-world eve archetype for Healthcare Ops.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/42-healthcare-intake-summarizer 3242 "Review the current healthcare intake summarizer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 42-healthcare-intake-summarizer
```
...
> Review the current healthcare intake summarizer queue and write a prioritized action report.
...
cd agents/catalog/42-healthcare-intake-summarizer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
