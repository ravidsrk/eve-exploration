...
Agent fleet router is a real-world eve archetype for Operations.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/50-agent-fleet-router 3250 "Review the current agent fleet router queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 50-agent-fleet-router
```
...
> Review the current agent fleet router queue and write a prioritized action report.
...
cd agents/catalog/50-agent-fleet-router && npx eve eval --strict   # needs keys in .secrets/eve.env
```
