...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/35-roadmap-prioritizer 3235 "Review the current roadmap prioritizer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 35-roadmap-prioritizer
```
...
cd agents/catalog/35-roadmap-prioritizer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
