...
Access request approver is a real-world eve archetype for Security.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/31-access-request-approver 3231 "Review the current access request approver queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 31-access-request-approver
```
...
> Review the current access request approver queue and write a prioritized action report.
...
cd agents/catalog/31-access-request-approver && npx eve eval --strict   # needs keys in .secrets/eve.env
```
