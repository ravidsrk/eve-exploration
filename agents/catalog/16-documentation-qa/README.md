...
Documentation QA is a real-world eve archetype for Developer Relations.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/16-documentation-qa 3216 "Review the current documentation qa queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 16-documentation-qa
```
...
> Review the current documentation qa queue and write a prioritized action report.
...
cd agents/catalog/16-documentation-qa && npx eve eval --strict   # needs keys in .secrets/eve.env
```
