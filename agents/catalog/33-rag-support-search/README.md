...
RAG support search is a real-world eve archetype for Support.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/33-rag-support-search 3233 "Review the current rag support search queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 33-rag-support-search
```
...
> Review the current rag support search queue and write a prioritized action report.
...
cd agents/catalog/33-rag-support-search && npx eve eval --strict   # needs keys in .secrets/eve.env
```
