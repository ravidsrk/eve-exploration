...
Market/news briefing is a real-world eve archetype for Strategy.

Mission: Generates daily sector brief with confidence and source gaps.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/21-market-news-briefing 3221 "Review the current market/news briefing queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 21-market-news-briefing
```
...
> Review the current market/news briefing queue and write a prioritized action report.
...
cd agents/catalog/21-market-news-briefing && npx eve eval --strict   # needs keys in .secrets/eve.env
```
