# Invoice anomaly auditor

## Rationale

Invoice anomaly auditor is a real-world eve archetype for Finance Ops.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/23-invoice-anomaly-auditor 3223 "Review the current invoice anomaly auditor queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 23-invoice-anomaly-auditor
```
...
> Review the current invoice anomaly auditor queue and write a prioritized action report.
...
cd agents/catalog/23-invoice-anomaly-auditor && npx eve eval --strict   # needs keys in .secrets/eve.env
```
