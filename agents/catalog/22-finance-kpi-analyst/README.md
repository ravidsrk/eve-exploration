...
Finance KPI analyst is a real-world eve archetype for Finance.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/22-finance-kpi-analyst 3222 "Review the current finance kpi analyst queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 22-finance-kpi-analyst
```
...
> Review the current finance kpi analyst queue and write a prioritized action report.
...
cd agents/catalog/22-finance-kpi-analyst && npx eve eval --strict   # needs keys in .secrets/eve.env
```
