# PDF due diligence analyst

## Rationale

PDF due diligence analyst is a real-world eve archetype for Corporate Development.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/40-pdf-due-diligence-analyst 3240 "Review the current pdf due diligence analyst queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 40-pdf-due-diligence-analyst
```
...
> Review the current pdf due diligence analyst queue and write a prioritized action report.
...
cd agents/catalog/40-pdf-due-diligence-analyst && npx eve eval --strict   # needs keys in .secrets/eve.env
```
