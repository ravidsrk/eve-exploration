...
Sales lead researcher is a real-world eve archetype for Sales.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/02-sales-lead-researcher 3202 "Review the current sales lead researcher queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 02-sales-lead-researcher
```
...
> Review the current sales lead researcher queue and write a prioritized action report.
...
cd agents/catalog/02-sales-lead-researcher && npx eve eval --strict   # needs keys in .secrets/eve.env
```
