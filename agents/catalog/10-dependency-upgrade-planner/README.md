...
Dependency upgrade planner is a real-world eve archetype for Engineering.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/10-dependency-upgrade-planner 3210 "Review the current dependency upgrade planner queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 10-dependency-upgrade-planner
```
...
> Review the current dependency upgrade planner queue and write a prioritized action report.
...
cd agents/catalog/10-dependency-upgrade-planner && npx eve eval --strict   # needs keys in .secrets/eve.env
```
