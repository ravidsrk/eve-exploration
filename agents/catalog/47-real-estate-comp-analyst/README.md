# Real estate comp analyst

## Rationale

Real estate comp analyst is a real-world eve archetype for Real Estate.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/47-real-estate-comp-analyst 3247 "Review the current real estate comp analyst queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 47-real-estate-comp-analyst
```
...
> Review the current real estate comp analyst queue and write a prioritized action report.
...
cd agents/catalog/47-real-estate-comp-analyst && npx eve eval --strict   # needs keys in .secrets/eve.env
```
