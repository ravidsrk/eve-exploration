...
CRM hygiene auditor is a real-world eve archetype for RevOps.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/03-crm-hygiene-auditor 3203 "Review the current crm hygiene auditor queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 03-crm-hygiene-auditor
```
...
> Review the current crm hygiene auditor queue and write a prioritized action report.
...
cd agents/catalog/03-crm-hygiene-auditor && npx eve eval --strict   # needs keys in .secrets/eve.env
```
