...
Migration planner is a real-world eve archetype for Engineering.

Mission: Converts legacy API/code inventory into an execution plan.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/14-migration-planner 3214 "Review the current migration planner queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 14-migration-planner
```
...
> Review the current migration planner queue and write a prioritized action report.
...
cd agents/catalog/14-migration-planner && npx eve eval --strict   # needs keys in .secrets/eve.env
```
