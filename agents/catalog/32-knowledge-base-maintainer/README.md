...
Knowledge base maintainer is a real-world eve archetype for Support Ops.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/32-knowledge-base-maintainer 3232 "Review the current knowledge base maintainer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 32-knowledge-base-maintainer
```
...
> Review the current knowledge base maintainer queue and write a prioritized action report.
...
cd agents/catalog/32-knowledge-base-maintainer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
