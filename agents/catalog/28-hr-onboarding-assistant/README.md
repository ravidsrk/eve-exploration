...
HR onboarding assistant is a real-world eve archetype for People Ops.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/28-hr-onboarding-assistant 3228 "Review the current hr onboarding assistant queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 28-hr-onboarding-assistant
```
...
> Review the current hr onboarding assistant queue and write a prioritized action report.
...
cd agents/catalog/28-hr-onboarding-assistant && npx eve eval --strict   # needs keys in .secrets/eve.env
```
