...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/48-education-tutor-planner 3248 "Review the current education tutor planner queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 48-education-tutor-planner
```
...
cd agents/catalog/48-education-tutor-planner && npx eve eval --strict   # needs keys in .secrets/eve.env
```
