# Privacy DSR responder

## Rationale

Privacy DSR responder is a real-world eve archetype for Privacy.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/27-privacy-dsr-responder 3227 "Review the current privacy dsr responder queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 27-privacy-dsr-responder
```
...
> Review the current privacy dsr responder queue and write a prioritized action report.
...
cd agents/catalog/27-privacy-dsr-responder && npx eve eval --strict   # needs keys in .secrets/eve.env
```
