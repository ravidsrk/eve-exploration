...
Experiment analyst is a real-world eve archetype for Growth.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/36-experiment-analyst 3236 "Review the current experiment analyst queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 36-experiment-analyst
```
...
> Review the current experiment analyst queue and write a prioritized action report.
...
cd agents/catalog/36-experiment-analyst && npx eve eval --strict   # needs keys in .secrets/eve.env
```
