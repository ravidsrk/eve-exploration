...
Competitor intelligence brief is a real-world eve archetype for Strategy.

Mission: Produces sourced competitor/product movement summaries.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/20-competitor-intelligence-brief 3220 "Review the current competitor intelligence brief queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 20-competitor-intelligence-brief
```
...
> Review the current competitor intelligence brief queue and write a prioritized action report.
...
cd agents/catalog/20-competitor-intelligence-brief && npx eve eval --strict   # needs keys in .secrets/eve.env
```
...
Label source gaps and stale evidence explicitly.
