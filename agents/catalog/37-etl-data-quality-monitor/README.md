...
ETL data quality monitor is a real-world eve archetype for Data.

Mission: Detects schema drift, null spikes, and late data.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/37-etl-data-quality-monitor 3237 "Review the current etl data quality monitor queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 37-etl-data-quality-monitor
```
...
> Review the current etl data quality monitor queue and write a prioritized action report.
...
cd agents/catalog/37-etl-data-quality-monitor && npx eve eval --strict   # needs keys in .secrets/eve.env
```
