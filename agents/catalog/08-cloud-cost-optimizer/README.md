...
Cloud cost optimizer is a real-world eve archetype for Platform.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/08-cloud-cost-optimizer 3208 "Review the current cloud cost optimizer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 08-cloud-cost-optimizer
```
...
> Review the current cloud cost optimizer queue and write a prioritized action report.
...
cd agents/catalog/08-cloud-cost-optimizer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
