...
Inventory replenishment planner is a real-world eve archetype for Operations.

Mission: Recommends reorder actions from demand and lead time.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/46-inventory-replenishment-planner 3246 "Review the current inventory replenishment planner queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 46-inventory-replenishment-planner
```
...
> Review the current inventory replenishment planner queue and write a prioritized action report.
...
cd agents/catalog/46-inventory-replenishment-planner && npx eve eval --strict   # needs keys in .secrets/eve.env
```
