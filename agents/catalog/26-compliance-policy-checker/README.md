...
Compliance policy checker is a real-world eve archetype for Compliance.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/26-compliance-policy-checker 3226 "Review the current compliance policy checker queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 26-compliance-policy-checker
```
...
> Review the current compliance policy checker queue and write a prioritized action report.
...
cd agents/catalog/26-compliance-policy-checker && npx eve eval --strict   # needs keys in .secrets/eve.env
```
