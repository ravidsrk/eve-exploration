# Code review risk assessor

## Rationale

Code review risk assessor is a real-world eve archetype for Engineering.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/12-code-review-risk-assessor 3212 "Review the current code review risk assessor queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 12-code-review-risk-assessor
```
...
> Review the current code review risk assessor queue and write a prioritized action report.
...
cd agents/catalog/12-code-review-risk-assessor && npx eve eval --strict   # needs keys in .secrets/eve.env
```
