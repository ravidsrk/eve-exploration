...
Research literature mapper is a real-world eve archetype for Research.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/49-research-literature-mapper 3249 "Review the current research literature mapper queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 49-research-literature-mapper
```
...
> Review the current research literature mapper queue and write a prioritized action report.
...
cd agents/catalog/49-research-literature-mapper && npx eve eval --strict   # needs keys in .secrets/eve.env
```
