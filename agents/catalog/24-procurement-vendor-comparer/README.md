...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/24-procurement-vendor-comparer 3224 "Review the current procurement vendor comparer queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 24-procurement-vendor-comparer
```
...
cd agents/catalog/24-procurement-vendor-comparer && npx eve eval --strict   # needs keys in .secrets/eve.env
```
