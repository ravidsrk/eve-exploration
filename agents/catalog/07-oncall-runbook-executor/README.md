...
On-call runbook executor is a real-world eve archetype for SRE.
...
```bash
bash ../../scripts/run-catalog-agent.sh agents/catalog/07-oncall-runbook-executor 3207 "Review the current on-call runbook executor queue and write a prioritized action report."
```
...
```bash
npm run deploy:catalog -- 07-oncall-runbook-executor
```
...
> Review the current on-call runbook executor queue and write a prioritized action report.
...
cd agents/catalog/07-oncall-runbook-executor && npx eve eval --strict   # needs keys in .secrets/eve.env
```
