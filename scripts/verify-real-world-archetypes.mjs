...
const dirs = readdirSync(archetypesDir, { withFileTypes: true })
  .filter((entry) => entry.isDirectory())
  .map((entry) => entry.name)
  .filter((name) => /^\d{2}-/.test(name))
  .sort();

if (dirs.length !== 50) fail(`expected 50 archetype directories, found ${dirs.length}`);

for (const dir of dirs) {
  const full = path.join(archetypesDir, dir);
...
if (!process.exitCode) {
  console.log(`PASS: verified ${dirs.length} real-world eve archetypes`);
}
