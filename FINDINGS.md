...
5. **Scoped workspace names confuse `eve eval`.** A package named `@arch/06-multi-tool` is reported by the running agent as `06-multi-tool`, but the eval runner expects the package name verbatim → "Expected eval target … but … is responding." Unscoping the package name fixed it.
